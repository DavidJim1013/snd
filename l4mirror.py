...
class L4Mirror14(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if msg.msg_len < msg.total_len:
            return
        acts = [psr.OFPActionOutput(ofp.OFPP_CONTROLLER, ofp.OFPCML_NO_BUFFER)]
        match = psr.OFPMatch(in_port=in_port, ipv4_src=iph.src, ipv4_dst=iph.dst, tcp_src=tcph.src, tcp_dst=tcph.dst)
        self.add_flow(dp, 1, match, acts)
        acts = [psr.OFPActionOutput(3)]
        self.add_flow(dp, 1, match, acts)
        out = psr.OFPPacketOut(datapath=dp, buffer_id=msg.buffer_id,
                               in_port=in_port, actions=acts, data=msg.data[0:10])
        dp.send_msg(out)
        self.ht.append((eth.src, eth.dst, in_port, 3))
        acts = [psr.OFPActionOutput(out_port)]
        self.add_flow(dp, 1, match, acts)
        self.ht.append((eth.src, eth.dst, in_port, out_port))
        if in_port == 2:
            acts = None
        #
